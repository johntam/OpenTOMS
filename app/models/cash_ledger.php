...
class CashLedger extends AppModel {
	...
	//get the carried forward cash balance
	...
	function getCash($fund, $date, $ccy) {
		...
		$cashdata = $this->find('all', array( 'fields'=>array(	'CashLedger.trade_date',
												...
												'conditions'=>array('CashLedger.fund_id =' => $fund,
																	...
																	'OR' => array('CashLedger.ledger_date =' => $date,
																				  'CashLedger.settlement_date >' => $date),	//settlement date basis
		...
		//for all the unsettled trades, swap the debits and credits around because we are adding them back into the cash
		$unsett = array();
		foreach ($cashdata as $key=>$c) {
			if ((strtotime($c['CashLedger']['settlement_date']) > strtotime($date)) && ($c['CashLedger']['ledger_date'] == $date)) {
				unset($cashdata[$key]);
				
				/*
				$unsett[] = array(	'CashLedger' => array(	'trade_date'=> $c['CashLedger']['trade_date'],
															'settlement_date'=> $c['CashLedger']['settlement_date'],
															'ledger_debit'=> $c['CashLedger']['ledger_credit'],
															'ledger_credit'=> $c['CashLedger']['ledger_debit'],
															'ledger_quantity'=> -1 * $c['CashLedger']['ledger_quantity']),
									'Trade' =>		array(	'id' => $c['Trade']['id']),
									'Sec2' =>		array(	'sec_name' => $c['Sec2']['sec_name'].' (unsettled)'));
									
				$cashdata[$key]['Sec2']['sec_name'] .= ' (unsettled)';
				*/
			}
		}
		$cashdata = $cashdata + $unsett;
	...
	//calculate the value of unsettled trades at the cash ledger date
	function calcUnsettled($fund, $date, $ccy) {
